...
`core.process.run` and `core.ds.stream` return `NAV` when given XF-language functions.

Proper fix requires:

* each worker thread owning its own `Interp`
* shared `VM`
* copied symbol scope
...
# v0.9.10 — Data Semantics Freeze

Goal: **stabilize data model and pipeline composition**

### Fix dataset shape mismatch

Preferred solution:

Change `core.process.assign` to return:
...
* consistent with `ds.flatten`
* simpler pipeline composition
* simpler API
* no extra flatten branches required

Alternative (less desirable):
extend `ds.flatten` to support `{columns, rows}`.

### Lock container iteration semantics
...
| tuple     | element | index, element   |
| map       | key     | key, value       |
| set       | element | element, element |

Your current set behavior:

```
s[x]   → x = element
s[i,x] → i = element, x = element
```
...
* destructuring non-tuple errors

### Exit criteria

* dataset pipelines compose naturally
* collection iteration semantics finalized
