...
static xf_Value ds_cell(xf_map_t *row, const char *key) {
    ...
    xf_Value v = xf_map_get(row, ks); xf_str_release(ks); return v;
}
...
static int ds_val_cmp(xf_Value a, xf_Value b) {
    if (a.state==XF_STATE_OK&&a.type==XF_TYPE_NUM&&b.state==XF_STATE_OK&&b.type==XF_TYPE_NUM) {
...
static xf_Value cd_row(xf_Value *args, size_t argc) {
    NEED(2);
    xf_arr_t *ds; if (!ds_arg_arr(args,argc,0,&ds)) return propagate(args,argc);
    double di; if (!arg_num(args,argc,1,&di)) return propagate(args,argc);
...
typedef struct { const char *key; int dir; } SortCtx;
...
static int ds_sort_cmp(const void *a, const void *b) {
    const xf_Value *ra=(const xf_Value *)a, *rb=(const xf_Value *)b;
...
static xf_Value cd_sort(xf_Value *args, size_t argc) {
    ...
    const char *col; size_t clen; if (!arg_str(args,argc,1,&col,&clen)) return propagate(args,argc);
    int dir=1;
    if (argc>=3){const char *d;size_t dl;if(arg_str(args,argc,2,&d,&dl)&&strncmp(d,"desc",4)==0)dir=-1;}
    xf_arr_t *out=xf_arr_new();
    for (size_t i=0;i<ds->len;i++) xf_arr_push(out,xf_value_retain(ds->items[i]));
    pthread_mutex_lock(&g_sort_mu);
    g_sort_ctx.key=col; g_sort_ctx.dir=dir;
...
static xf_Value cd_agg(xf_Value *args, size_t argc) {
    NEED(2);
    xf_arr_t *ds; if (!ds_arg_arr(args,argc,0,&ds)) return propagate(args,argc);
    const char *gkey; size_t glen; if (!arg_str(args,argc,1,&gkey,&glen)) return propagate(args,argc);
    const char *akey=NULL; size_t alen=0;
    bool has_akey=(argc>=3&&arg_str(args,argc,2,&akey,&alen));
    xf_map_t *out=xf_map_new();
    for (size_t i=0;i<ds->len;i++) {
        xf_map_t *row=ds_row_map(ds,i); if (!row) continue;
        xf_Value gval=ds_cell(row,gkey), gs=xf_coerce_str(gval);
        if (gs.state!=XF_STATE_OK||!gs.data.str) continue;
        xf_Str *gstr=gs.data.str;
        xf_Value bucket=xf_map_get(out,gstr); xf_arr_t *ba;
        if (bucket.state!=XF_STATE_OK||bucket.type!=XF_TYPE_ARR||!bucket.data.arr) {
            ba=xf_arr_new(); xf_map_set(out,gstr,xf_val_ok_arr(ba)); xf_arr_release(ba);
            ba=xf_map_get(out,gstr).data.arr;
        } else { ba=bucket.data.arr; }
        xf_Value push_val=has_akey?ds_cell(row,akey):ds->items[i];
        xf_arr_push(ba, xf_value_retain(push_val));
    }
    xf_Value rv=xf_val_ok_map(out); xf_map_release(out); return rv;
}
...
static xf_Value cd_merge(xf_Value *args, size_t argc) {
    ...
    if (!join_mode) {
        ...
        for (size_t i=0;i<ds1->len;i++) {
            ...
            xf_Value jv1=ds_cell(r1,jkey), js1=xf_coerce_str(jv1);
            xf_map_t *matched=NULL;
            if (js1.state==XF_STATE_OK&&js1.data.str) {
                for (size_t j=0;j<ds2->len;j++) {
                    xf_map_t *r2=ds_row_map(ds2,j); if (!r2) continue;
                    xf_Value jv2=ds_cell(r2,jkey), js2=xf_coerce_str(jv2);
                    if (js2.state==XF_STATE_OK&&js2.data.str&&
                        strcmp(js1.data.str->data,js2.data.str->data)==0){matched=r2;break;}
                }
            }
            if (!matched) { xf_arr_push(out,xf_value_retain(ds1->items[i])); }
            else {
                ...
                xf_Value mv=xf_val_ok_map(merged); xf_map_release(merged);
                xf_arr_push(out,mv);
            }
        }
    }
...
static xf_Value cd_index(xf_Value *args, size_t argc) {
    ...
    for (size_t i=0;i<ds->len;i++) {
        xf_map_t *row=ds_row_map(ds,i); if (!row) continue;
        xf_Value kv=ds_cell(row,col), ks=xf_coerce_str(kv);
        if (ks.state!=XF_STATE_OK||!ks.data.str) continue;
        if (xf_map_get(out,ks.data.str).state!=XF_STATE_OK)
            xf_map_set(out,ks.data.str,xf_value_retain(ds->items[i]));
    }
    xf_Value v=xf_val_ok_map(out); xf_map_release(out); return v;
}

static xf_Value cd_keys(xf_Value *args, size_t argc) {
    NEED(1);
    xf_arr_t *ds; if (!ds_arg_arr(args,argc,0,&ds)) return propagate(args,argc);
    xf_map_t *seen=xf_map_new(); xf_arr_t *out=xf_arr_new();
    for (size_t i=0;i<ds->len;i++) {
        xf_map_t *row=ds_row_map(ds,i); if (!row) continue;
...
static xf_Value cd_values(xf_Value *args, size_t argc) {
    ...
    for (size_t i=0;i<ds->len;i++) {
        ...
        xf_Value vv=xf_val_ok_arr(rv); xf_arr_push(out,vv); xf_arr_release(rv);
    }
    xf_Value v=xf_val_ok_arr(out); xf_arr_release(out); return v;
}
...
static xf_Value cd_filter(xf_Value *args, size_t argc) {
    ...
    const char *match_s=(has_val&&match_vs.state==XF_STATE_OK&&match_vs.data.str)?match_vs.data.str->data:NULL;
    xf_arr_t *out=xf_arr_new();
    for (size_t i=0;i<ds->len;i++) {
        xf_map_t *row=ds_row_map(ds,i); if (!row) continue;
...
static xf_Value cd_transpose(xf_Value *args, size_t argc) {
    NEED(1);
    xf_arr_t *ds; if (!ds_arg_arr(args,argc,0,&ds)) return propagate(args,argc);
    xf_map_t *seen=xf_map_new(); xf_arr_t *cols=xf_arr_new();
    for (size_t i=0;i<ds->len;i++) {
        xf_map_t *row=ds_row_map(ds,i); if (!row) continue;
        for (size_t k=0;k<row->order_len;k++) {
            xf_Str *kname=row->order[k];
    ...
    for (size_t c=0;c<cols->len;c++) {
        ...
        xf_Value cv=xf_val_ok_arr(col_arr); xf_map_set(out,cname,cv); xf_arr_release(col_arr);
    }
    xf_arr_release(cols);
    xf_Value v=xf_val_ok_map(out); xf_map_release(out); return v;
}
...
static xf_Value cd_expand(xf_Value *args, size_t argc) {
    ...
    for (size_t i=0;i<cols->order_len;i++) {
        ...
        if (cv.state==XF_STATE_OK&&cv.type==XF_TYPE_ARR&&cv.data.arr&&cv.data.arr->len>rows)
    ...
    for (size_t r=0;r<rows;r++) {
        ...
        for (size_t c=0;c<cols->order_len;c++) {
            ...
            if (cv.state==XF_STATE_OK&&cv.type==XF_TYPE_ARR&&cv.data.arr) {
                xf_Value cell=(r<cv.data.arr->len)?xf_value_retain(cv.data.arr->items[r]):xf_val_nav(XF_TYPE_VOID);
                xf_map_set(row,k,cell);
            }
        }
        xf_Value rv=xf_val_ok_map(row); xf_arr_push(out,rv); xf_map_release(row);
    }
    xf_Value v=xf_val_ok_arr(out); xf_arr_release(out); return v;
}
        ...
        if (v.state==XF_STATE_OK&&v.type==XF_TYPE_ARR&&v.data.arr) {
            if (deep) {
                ...
                if (sub.state==XF_STATE_OK&&sub.type==XF_TYPE_ARR&&sub.data.arr) {
                    for (size_t j=0;j<sub.data.arr->len;j++) xf_arr_push(out,xf_value_retain(sub.data.arr->items[j]));
                    xf_value_release(sub);
                }
            } else {
                for (size_t j=0;j<v.data.arr->len;j++) xf_arr_push(out,xf_value_retain(v.data.arr->items[j]));
            }
        } else { xf_arr_push(out,xf_value_retain(v)); }
    }
...
static xf_Value cd_flatten(xf_Value *args, size_t argc) {
    ...
    if (src.type==XF_TYPE_ARR&&src.data.arr) {
        ...
        if (in->len==0){xf_arr_t *e=xf_arr_new();xf_Value v=xf_val_ok_arr(e);xf_arr_release(e);return v;}

        /* inverted index maps detection */
        bool all_index_maps=(in->len>0);
        for (size_t i=0;i<in->len&&all_index_maps;i++) {
            xf_Value v=in->items[i];
            if (!(v.state==XF_STATE_OK&&v.type==XF_TYPE_MAP&&v.data.map)){all_index_maps=false;break;}
            xf_map_t *m=v.data.map; if (m->order_len==0){all_index_maps=false;break;}
            for (size_t j=0;j<m->order_len&&all_index_maps;j++) {
                xf_Value inner=xf_map_get(m,m->order[j]);
                if (!(inner.state==XF_STATE_OK&&inner.type==XF_TYPE_MAP)) all_index_maps=false;
            }
        }
        if (all_index_maps) {
            xf_map_t *merged=xf_map_new();
            for (size_t i=0;i<in->len;i++) {
                ...
                for (size_t c=0;c<chunk_idx->order_len;c++) {
                    ...
                    if (col_src.state!=XF_STATE_OK||col_src.type!=XF_TYPE_MAP||!col_src.data.map) continue;
                    xf_map_t *src_col=col_src.data.map;
                    xf_Value dst_col_v=xf_map_get(merged,col_key); xf_map_t *dst_col;
                    if (dst_col_v.state!=XF_STATE_OK||dst_col_v.type!=XF_TYPE_MAP||!dst_col_v.data.map) {
                        dst_col=xf_map_new(); xf_map_set(merged,col_key,xf_val_ok_map(dst_col));
                        xf_map_release(dst_col); dst_col=xf_map_get(merged,col_key).data.map;
                    } else { dst_col=dst_col_v.data.map; }
                    for (size_t v2=0;v2<src_col->order_len;v2++) {
                        xf_Str *val_key=src_col->order[v2];
                        xf_Value src_ids=xf_map_get(src_col,val_key);
                        if (src_ids.state!=XF_STATE_OK||src_ids.type!=XF_TYPE_ARR||!src_ids.data.arr) continue;
                        xf_Value dst_ids_v=xf_map_get(dst_col,val_key); xf_arr_t *dst_ids;
                        if (dst_ids_v.state!=XF_STATE_OK||dst_ids_v.type!=XF_TYPE_ARR||!dst_ids_v.data.arr) {
                            dst_ids=xf_arr_new(); xf_map_set(dst_col,val_key,xf_val_ok_arr(dst_ids));
                            xf_arr_release(dst_ids); dst_ids=xf_map_get(dst_col,val_key).data.arr;
                        } else { dst_ids=dst_ids_v.data.arr; }
                        xf_arr_t *sa=src_ids.data.arr;
                        for (size_t id=0;id<sa->len;id++) xf_arr_push(dst_ids,xf_value_retain(sa->items[id]));
                    }
                }
            }
            xf_Value r=xf_val_ok_map(merged); xf_map_release(merged); return r;
        }
        ...
        for (size_t i=0;i<in->len&&all_arr_of_map;i++) {
            xf_Value v=in->items[i];
            if (!(v.state==XF_STATE_OK&&v.type==XF_TYPE_ARR&&v.data.arr)){all_arr_of_map=false;break;}
            for (size_t j=0;j<v.data.arr->len&&all_arr_of_map;j++) {
                xf_Value inner=v.data.arr->items[j];
                if (!(inner.state==XF_STATE_OK&&inner.type==XF_TYPE_MAP)) all_arr_of_map=false;
            }
        }
        if (all_arr_of_map) {
            xf_arr_t *out=xf_arr_new();
            for (size_t i=0;i<in->len;i++) {
                ...
                for (size_t j=0;j<chunk->len;j++) xf_arr_push(out,xf_value_retain(chunk->items[j]));
            }
            xf_Value v=xf_val_ok_arr(out); xf_arr_release(out); return v;
        }
            ...
            for (size_t i=0;i<in->order_len;i++) {
                ...
                if (v.state==XF_STATE_OK&&v.type==XF_TYPE_MAP&&v.data.map) {
                    ...
                    for (size_t j=0;j<sm->order_len;j++)
                        xf_map_set(out,sm->order[j],xf_value_retain(xf_map_get(sm,sm->order[j])));
                }
            }
            xf_Value r=xf_val_ok_map(out); xf_map_release(out); return r;
        }
        xf_arr_t *out=xf_arr_new();
        for (size_t i=0;i<in->order_len;i++) xf_arr_push(out,xf_value_retain(xf_map_get(in,in->order[i])));
        xf_Value r=xf_val_ok_arr(out); xf_arr_release(out); return r;
    }
    xf_arr_t *out=xf_arr_new(); xf_arr_push(out,xf_value_retain(src));
    xf_Value r=xf_val_ok_arr(out); xf_arr_release(out); return r;
}
...
static void *cd_pagg_thread(void *arg) {
    ...
    xf_arr_t *ds=ctx->chunk; xf_fn_t *fn=ctx->fn;
    ...
    for (size_t i=0;i<ds->len;i++) {
        ...
        if (fn) {
            ...
            if (fn->is_native&&fn->native_v) { xformed=fn->native_v(&row_v,1); }
            else {
            ...
            if (xformed.state==XF_STATE_OK&&xformed.type==XF_TYPE_MAP) row_v=xformed;
        }
        xf_map_t *row=(row_v.state==XF_STATE_OK&&row_v.type==XF_TYPE_MAP)?row_v.data.map:NULL;
        if (!row) continue;
        xf_Value gval=ds_cell(row,ctx->gkey), gs=xf_coerce_str(gval);
        ...
        xf_arr_push(ba,xf_value_retain(push_val)); xf_value_release(gs);
    }
    ctx->result=out; ctx->done=true; return NULL;
}
...
static xf_Value cd_agg_parallel(xf_Value *args, size_t argc) {
    ...
    const char *gkey; size_t glen; if (!arg_str(args,argc,1,&gkey,&glen)) return propagate(args,argc);
    const char *akey=NULL; size_t alen=0;
    if (argc>=3&&args[2].state==XF_STATE_OK&&args[2].type==XF_TYPE_STR) arg_str(args,argc,2,&akey,&alen);
    xf_fn_t *fn=NULL;
    if (argc>=4&&args[3].state==XF_STATE_OK&&args[3].type==XF_TYPE_FN) fn=args[3].data.fn;
    double dn=4.0;
    if (argc>=4&&args[3].state==XF_STATE_OK&&args[3].type==XF_TYPE_NUM) dn=args[3].data.num;
    else if (argc>=5) arg_num(args,argc,4,&dn);
    ...
    PaggCtx ctxs[CD_PAGG_MAX]; pthread_t tids[CD_PAGG_MAX]; xf_arr_t *chunks[CD_PAGG_MAX];
    xf_fn_caller_t caller=core_get_fn_caller();
    ...
    for (size_t i=0;i<n;i++) {
        ...
        ctxs[nthreads]=(PaggCtx){.chunk=chunk,.gkey=gkey,.akey=akey,.fn=fn,.result=NULL,
    ...
    for (size_t i=0;i<nthreads;i++) {
        ...
        for (size_t k=0;k<partial->order_len;k++) {
            ...
            if (src_bkt.state!=XF_STATE_OK||src_bkt.type!=XF_TYPE_ARR||!src_bkt.data.arr) continue;
            xf_Value dst_bkt=xf_map_get(merged,gstr); xf_arr_t *da;
            if (dst_bkt.state!=XF_STATE_OK||dst_bkt.type!=XF_TYPE_ARR||!dst_bkt.data.arr) {
                da=xf_arr_new(); xf_map_set(merged,gstr,xf_val_ok_arr(da));
                xf_arr_release(da); da=xf_map_get(merged,gstr).data.arr;
            } else { da=dst_bkt.data.arr; }
            xf_arr_t *sa=src_bkt.data.arr;
            for (size_t j=0;j<sa->len;j++) xf_arr_push(da,xf_value_retain(sa->items[j]));
        }
        xf_map_release(partial); xf_arr_release(chunks[i]);
    }
    xf_Value tmp=xf_val_ok_map(merged); xf_map_release(merged); return tmp;
}
...
static void *cd_stream_thread(void *arg) {
    StreamCtx *ctx=(StreamCtx *)arg; xf_fn_t *fn=ctx->fn;
    xf_Value chunk_val=xf_val_ok_arr(ctx->chunk);
    if (ctx->caller) core_set_fn_caller(ctx->caller_vm,ctx->caller_syms,ctx->caller);
    if (fn->is_native&&fn->native_v) { ctx->result=fn->native_v(&chunk_val,1); }
    else {
        xf_fn_caller_t caller=core_get_fn_caller();
        void *vm=core_get_fn_caller_vm(), *sy=core_get_fn_caller_syms();
        ctx->result=(caller&&vm)?caller(vm,sy,fn,&chunk_val,1):xf_val_nav(XF_TYPE_FN);
    }
    xf_value_release(chunk_val); ctx->done=true; return NULL;
}

static xf_arr_t *cd_stream_read_file(const char *path) {
    FILE *fp=fopen(path,"r"); xf_arr_t *out=xf_arr_new(); if (!fp) return out;
    char line[65536]; size_t nr=0;
    xf_Str *k_line=xf_str_from_cstr("line"),*k_nr=xf_str_from_cstr("nr"),
            *k_file=xf_str_from_cstr("file"),*vfile=xf_str_from_cstr(path);
    while (fgets(line,sizeof(line),fp)) {
        size_t len=strlen(line);
        while (len>0&&(line[len-1]=='\n'||line[len-1]=='\r')) line[--len]='\0';
        xf_map_t *row=xf_map_new(); xf_Str *vline=xf_str_new(line,len);
        ...
        xf_arr_push(out,xf_val_ok_map(row)); xf_map_release(row);
    }
    fclose(fp);
    xf_str_release(k_line); xf_str_release(k_nr); xf_str_release(k_file); xf_str_release(vfile);
    return out;
}
...
static xf_Value cd_stream(xf_Value *args, size_t argc) {
    NEED(2);
    if (args[0].state!=XF_STATE_OK||args[0].type!=XF_TYPE_ARR||!args[0].data.arr) return xf_val_nav(XF_TYPE_ARR);
    if (args[1].state!=XF_STATE_OK||args[1].type!=XF_TYPE_FN||!args[1].data.fn)   return xf_val_nav(XF_TYPE_ARR);
    xf_arr_t *sources=args[0].data.arr; xf_fn_t *fn=args[1].data.fn;
    size_t nsrc=sources->len<CD_STREAM_MAX?sources->len:CD_STREAM_MAX;
    StreamCtx *ctxs=calloc(nsrc,sizeof(StreamCtx)); pthread_t *tids=calloc(nsrc,sizeof(pthread_t));
    xf_fn_caller_t caller=core_get_fn_caller();
    void *caller_vm=core_get_fn_caller_vm(), *caller_syms=core_get_fn_caller_syms();
    for (size_t i=0;i<nsrc;i++) {
        xf_Value sv=sources->items[i]; xf_arr_t *chunk=NULL;
        ...
        else { chunk=xf_arr_new(); xf_arr_push(chunk,xf_value_retain(sv)); }
        ctxs[i]=(StreamCtx){.chunk=chunk,.fn=fn,.caller=caller,.caller_vm=caller_vm,
    ...
    for (size_t i=0;i<nsrc;i++) {
        pthread_join(tids[i],NULL); xf_Value r=ctxs[i].result;
        if (r.state==XF_STATE_OK&&r.type==XF_TYPE_ARR&&r.data.arr) {
            for (size_t j=0;j<r.data.arr->len;j++) xf_arr_push(collected,xf_value_retain(r.data.arr->items[j]));
            xf_value_release(r);
        } else { xf_arr_push(collected,r); }
        xf_arr_release(ctxs[i].chunk);
    }
    free(ctxs); free(tids);
    xf_Value v=xf_val_ok_arr(collected); xf_arr_release(collected); return v;
}
